...
int main(int argc, char** argv) {
  ...
  if(argc < 3) {
    ...
    printf("  N: number of threads (not counting monitor core, 8 core carp need this to be 7)\n");
    printf("  S: Not sure what to do with this parameter\n");
    printf("  per_thread_increment: number of increments performed per thread\n");
    fflush(stdout);
    return -1;
  }
  ...
  for(i = 0; i < n_thread; i++) {
    monaddr[i] = (unsigned long)&shared_variables[i % S];
  }
