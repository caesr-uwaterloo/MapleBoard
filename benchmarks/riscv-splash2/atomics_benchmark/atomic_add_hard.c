...
long long per_thread_increment = 20000000;
...
int main() {
  ...
  pthread_create(&thread[2], NULL, monitorFunc, (void*)1);
  ...
  int n = 3;
  if(shared_variable != n * per_thread_increment) {
    printf("***** Failed: expected %lld, actual %lld *****\n", n * per_thread_increment, shared_variable);
  } else {
