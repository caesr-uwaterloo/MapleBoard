...
class Memory(coreParam: CoreParam) extends Module {
  ...
  /**
    * Memory stage ready valid signal generation block
    */
  io.controlInput.ready := reqQ.io.enq.ready
  io.dataInput.ready := io.controlInput.ready
  io.controlOutput.valid := respQ.io.deq.valid || (io.controlInput.valid && (!io.controlInput.bits.isMemory || misaligned))
  io.dataOutput.valid := io.controlOutput.valid

  reqQ.io.enq.valid := io.controlInput.bits.isMemory && io.controlInput.valid && !misaligned
  ...
  when(io.controlInput.valid) {
    printf(p"[M${coreParam.coreID}] respQ deque valid: ${respQ.io.deq.valid}, reqQenque valid: ${reqQ.io.enq.valid}, misaligned: ${misaligned}\n")
    ...
    printf(p"[M${coreParam.coreID}] pc: ${Hexadecimal(io.dataOutput.bits.pc)}, inst: ${Hexadecimal(io.dataOutput.bits.inst)}, isMemory? ${io.controlInput.bits.isMemory}, controloutput valid? ${io.controlOutput.valid}\n")
  }
