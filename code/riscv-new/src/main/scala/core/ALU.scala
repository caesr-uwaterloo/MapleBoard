...
// the usage of ChiselEnum can be found at:
// https://github.com/freechipsproject/chisel3/blob/master/src/test/scala/cookbook/FSM.scala
...
class ALU(isaParam: RISCVParam) extends Module {
  ...
  if(isaParam.XLEN > 32) {
    ...
    shamt(shamtWidth - 1) := (shamt(shamtWidth - 1) & io.isW).asUInt
  }
  */
  ...
  if(isaParam.XLEN > 32) {
    when(io.isW) {
      require(isaParam.XLEN == 64)
      io.out := Cat(Fill(32, out(w)), out(w, 0))
    }
  }
}
