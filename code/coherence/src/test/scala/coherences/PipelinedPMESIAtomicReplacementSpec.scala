...
import components._
import fixtures.TargetPlatformPMESIAtomic
import org.scalatest.{FlatSpec, Matchers}
import testutil.{RequestManager, RequestSendConfig}

class PipelinedPMESIAtomicReplacementSpec extends FlatSpec with ChiselScalatestTester with Matchers {
  behavior of "PipelinedPMESIAtomic"
  ...
  it should "handle replacement correctly for shared everything" in {
    val fn = TargetPlatformPMESIAtomic.coherenceSpec.generatePrivateCacheTableFile(None)
    val fns = TargetPlatformPMESIAtomic.coherenceSpec.generateSharedCacheTableFile(None)
    println(s"The Private Cache Coherence Table is at ${fn}")
    val memParam = TargetPlatformPMESIAtomic.memorySystemParams.copy(masterCount = 4)
    test(new PipelinedBareMemorySubsystem(
      TargetPlatformPMESIAtomic.coreParam,
      memParam,
      TargetPlatformPMESIAtomic.coherenceSpec)).withAnnotations(Seq(VerilatorBackendAnnotation, VerilatorFlags(Seq("-DPRINTF_COND=1")), WriteVcdAnnotation)) { c =>
