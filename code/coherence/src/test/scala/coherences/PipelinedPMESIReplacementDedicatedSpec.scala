...
import components._
import fixtures.TargetPlatformPMESIDedicated
import org.scalatest.{FlatSpec, Matchers}
import testutil.{RequestManager, RequestSendConfig}

class PipelinedPMESIReplacementDedicatedSpec extends FlatSpec with ChiselScalatestTester with Matchers {
  behavior of "PipelinedPMESIDedicated"
  ...
  it should "handle replacement correctly for dedicated two way" in {
    val memParam = TargetPlatformPMESIDedicated.memorySystemParams.copy(masterCount = 8)
    memParam.getDataBusConf match {
      case DedicatedDataBusTwoWay => {}
      case _ => { assert(false) }
    }
    test(new PipelinedBareMemorySubsystem(
      TargetPlatformPMESIDedicated.coreParam,
      memParam,
      TargetPlatformPMESIDedicated.coherenceSpec)).withAnnotations(Seq(VerilatorBackendAnnotation, /* VerilatorFlags(Seq("-DPRINTF_COND=1")),  */WriteVcdAnnotation)) { c =>
  ...
  it should "handle replacement correctly for dedicated two way atomic" in {
    val memParam = TargetPlatformPMESIDedicated.memorySystemParams.copy(masterCount = 4)
    test(new PipelinedBareMemorySubsystem(
      TargetPlatformPMESIDedicated.coreParam,
      memParam,
      TargetPlatformPMESIDedicated.coherenceSpec)).withAnnotations(Seq(VerilatorBackendAnnotation, VerilatorFlags(Seq("-DPRINTF_COND=1")), WriteVcdAnnotation)) { c =>
