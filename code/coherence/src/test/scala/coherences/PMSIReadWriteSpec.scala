...
class PMSIReadWriteSpec extends FlatSpec with ChiselScalatestTester with Matchers {
  behavior of "PMSI read write"
  it should "give quadratic bound" in {
    ...
    test(new BareMemorySubsystem(
      ...
      memParam)).withAnnotations(Seq(VerilatorBackendAnnotation, VerilatorFlags(Seq("-DPRINTF_COND=0")), WriteVcdAnnotation)) { c =>
