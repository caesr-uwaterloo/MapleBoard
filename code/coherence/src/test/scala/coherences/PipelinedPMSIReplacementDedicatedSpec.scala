...
import fixtures.TargetPlatformPMSIDedicated
import org.scalatest.{FlatSpec, Matchers}
import testutil.{RequestManager, RequestSendConfig}
import chisel3._

class PipelinedPMSIReplacementDedicatedSpec extends FlatSpec with ChiselScalatestTester with Matchers {
  behavior of "PipelinedPMSIDedicated"
  ...
  it should "handle replacement correctly for dedicated two way" in {
    val fn = TargetPlatformPMSIDedicated.coherenceSpec.generatePrivateCacheTableFile(None)
    val fns = TargetPlatformPMSIDedicated.coherenceSpec.generateSharedCacheTableFile(None)
    println(s"The Private Cache Coherence Table is at ${fn}")
    val memParam = TargetPlatformPMSIDedicated.memorySystemParams.copy(masterCount = 4)
    memParam.getDataBusConf match {
      case DedicatedDataBusTwoWay => {}
      case _ => { assert(false) }
    }
    test(new PipelinedBareMemorySubsystem(
      TargetPlatformPMSIDedicated.coreParam,
      memParam,
      TargetPlatformPMSIDedicated.coherenceSpec)).withAnnotations(Seq(VerilatorBackendAnnotation, VerilatorFlags(Seq("-DPRINTF_COND=1")), WriteVcdAnnotation)) { c =>
