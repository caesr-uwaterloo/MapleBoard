...
import components._
import fixtures.TargetPlatformPMESIAtomicModified
import org.scalatest.{FlatSpec, Matchers}
import testutil.{RequestManager, RequestSendConfig}
import chisel3._

// This is only for the modified
class PipelinedPMESIAtomicModifiedReplacementSpec extends FlatSpec with ChiselScalatestTester with Matchers {
  behavior of "PipelinedPMESIAtomicModified"
  ...
  val pf = TargetPlatformPMESIAtomicModified
  // construct request. note that the address should be less
  require(pf.memorySystemParams.useAtomicBusModified)

  it should "handle replacement correctly for shared everything" in {
    val fn = pf.coherenceSpec.generatePrivateCacheTableFile(None)
    val fns = pf.coherenceSpec.generateSharedCacheTableFile(None)
    println(s"The Private Cache Coherence Table is at ${fn}")
    val memParam = pf.memorySystemParams.copy(masterCount = 8)
    test(new PipelinedBareMemorySubsystem(
      pf.coreParam,
      memParam,
      pf.coherenceSpec)).withAnnotations(Seq(VerilatorBackendAnnotation, VerilatorFlags(Seq("-DPRINTF_COND=1")), WriteVcdAnnotation)) { c =>
  ...
  it should "handle replacement correctly for shared everything lr/sc" in {
    ...
    val fns = pf.coherenceSpec.generateSharedCacheTableFile(None)
    println(s"The Private Cache Coherence Table is at ${fn}")
    val memParam = pf.memorySystemParams.copy(masterCount = 4)
    test(new PipelinedBareMemorySubsystem(
      pf.coreParam,
      memParam,
      pf.coherenceSpec)).withAnnotations(Seq(VerilatorBackendAnnotation, VerilatorFlags(Seq("-DPRINTF_COND=1")), WriteVcdAnnotation)) { c =>
      ...
      val r = new scala.util.Random(42)
      val addrs: List[Int] = List(0x40, 0x440, 0x840, 0xc40, 0x1040, 0x1440)
      // Now do some lr/sc requests
      val totalRequestPerCore = 50
