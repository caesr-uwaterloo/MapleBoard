...
class PipelinedPMSIReplacementSpec extends FlatSpec with ChiselScalatestTester with Matchers {
  behavior of "PipelinedPMSI"
  ...
  it should "handle replacement correctly for shared everything" in {
    ...
    test(new PipelinedBareMemorySubsystem(
      ...
      requestManager.testSendRecvRequest(req, RequestSendConfig(30, timeout = 160000))
    }
  }
  /*
