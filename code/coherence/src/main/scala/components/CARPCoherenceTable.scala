...
import coherences.CARP

class CARPCritCoherenceTable extends CoherenceTableGenerator(
  new CacheEvent { },
  new CARP {},
  new RequestType { }
) {
  ...
  override def table: Map[Event, Do]  = Map(
    ...
    Event(CE.OTHER_GETS,CohS.IM_D, CriticalityDiff.SAMECRIT) -> Do(CohS.IM_DUS, "UPDATE_CACHE"),
    Event(CE.OTHER_GETS,CohS.IM_D, CriticalityDiff.LOCRIT) -> Do(CohS.IM_DUS_E, "UPDATE_CACHE"),
    Event(CE.OTHER_GETM,CohS.IM_DUS) -> Do(CohS.IM_DUI, "UPDATE_CACHE"),
    Event(CE.OTHER_GETM,CohS.IM_DUS_E) -> Do(CohS.IM_DUI, "UPDATE_CACHE"),
    Event(CE.OTHER_GETS,CohS.IM_DUS_E, CriticalityDiff.SAMECRIT) -> Do(CohS.IM_DUS, "UPDATE_CACHE"),
    Event(CE.OTHER_GETS,CohS.IM_DUS_E, CriticalityDiff.LOCRIT) -> Do(CohS.IM_DUS_E, "UPDATE_CACHE"),
    ...
    Event(CE.DATA, CohS.IM_DUS_E) -> Do(CohS.IM_US_E, "REFILL_CACHE"),
    Event(CE.EDATA, CohS.IM_DUS_E) -> Do(CohS.IM_US_E, "REFILL_CACHE"),
    ...
    Event(CE.STORE, CohS.IM_US_E) -> Do(CohS.MS_A_E, "WRITE_DATA_ARRAY_ADD_TO_CACHE_RESP_Q_LO_CRIT").markDirty.broadcast(R.PUTM),
    ...
    Event(CE.EDATA, CohS.IS_D) -> Do(CohS.S, "REFILL_CACHE"), // nothing changes to E
    ...
    Event(CE.OTHER_GETS, CohS.M, CriticalityDiff.SAMECRIT) -> Do(CohS.MS_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),
    Event(CE.OTHER_GETM, CohS.M) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),

    /**
      * for cancelling Lo crit cores
      */
    Event(CE.OTHER_GETS, CohS.M, CriticalityDiff.LOCRIT) -> Do(CohS.MS_WB_E, "ADD_TO_LO_CRIT_WB_QUEUE").broadcast(R.PUTM),
    Event(CE.REPLACEMENT, CohS.MS_WB_E) -> Do(CohS.MI_WB_E, "UPDATE_CACHE"), // already enqueued
    Event(CE.OTHER_GETM, CohS.MS_WB_E) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),
    Event(CE.OTHER_GETS, CohS.MS_WB_E, CriticalityDiff.SAMECRIT) -> Do(CohS.MS_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),
    Event(CE.OTHER_GETM, CohS.MI_WB_E) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM), // need to broadcast again
    Event(CE.OTHER_GETS, CohS.MI_WB_E, CriticalityDiff.SAMECRIT) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),
    ...
    // resend the data the normal wb queue
    Event(CE.OTHER_GETS, CohS.IM_US_E, CriticalityDiff.SAMECRIT) -> Do(CohS.MS_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),
    /**
      * guarding...
      */
    ...
    Event(CE.OWN_PUTM, CohS.MS_A_E) -> Do(CohS.S, "UPDATE_CACHE").clean,
    Event(CE.OTHER_GETM, CohS.MS_WB) -> Do(CohS.MI_WB, "UPDATE_CACHE"),
    Event(CE.OTHER_GETM, CohS.MS_A) -> Do(CohS.MI_A, "UPDATE_CACHE"),
    Event(CE.OTHER_GETM, CohS.MS_A_E) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM), // we need to remove from LCWBQ
    Event(CE.OTHER_GETS, CohS.MS_A_E, CriticalityDiff.SAMECRIT) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM), // we need to remove from LCWBQ
  ...
  override def getCoherenceWBStates: List[Int] = List(CohS.MS_A, CohS.MI_A, CohS.MI_WB,
    CohS.MS_WB, CohS.MS_WB_E, CohS.MS_A_E/*, CohS.EI_A, CohS.ES_A*/)
}

class CARPNonCritCoherenceTable extends CoherenceTableGenerator(
  new CacheEvent { },
  new CARP {},
  new RequestType { }
) {
  ...
  override def table: Map[Event, Do]  = Map(
    // From I
    // Whatever to be in M, there's no need to guard
    Event(CE.LOAD,CohS.I) -> Do(CohS.IS_AD,"ADD_TO_REQ_QUEUE").markValid.broadcast(R.GETS),
    Event(CE.STORE,CohS.I) -> Do(CohS.IM_AD,"ADD_TO_REQ_QUEUE").markValid.broadcast(R.GETM),
    Event(CE.OWN_GETM,CohS.IM_AD) -> Do(CohS.IM_D, "UPDATE_CACHE"),
    Event(CE.OTHER_GETM,CohS.IM_D) -> Do(CohS.IM_DUI, "UPDATE_CACHE"),
    ...
    // Whatever to be in S, there's a need to guard!
    Event(CE.OWN_GETS, CohS.IS_AD) -> Do(CohS.IS_D, "UPDATE_CACHE"),

    Event(CE.OTHER_GETM, CohS.IS_D, CriticalityDiff.SAMECRIT) -> Do(CohS.IS_DI, "UPDATE_CACHE"),
    // Now goto re-issue the request
    Event(CE.OTHER_GETM, CohS.IS_D, CriticalityDiff.HICRIT) -> Do(CohS.IS_AD, "REISSUE").broadcast(R.GETS),
    Event(CE.OTHER_GETS, CohS.IS_D, CriticalityDiff.SAMECRIT) -> Do(CohS.IS_D, "UPDATE_CACHE"),
    // Now goto re-issue the request
    Event(CE.OTHER_GETS, CohS.IS_D, CriticalityDiff.HICRIT) -> Do(CohS.IS_AD, "REISSUE").broadcast(R.GETS),
    ...
    // We don't need to retry because this can be in Modified state
    Event(CE.OTHER_UPG, CohS.S) -> Do(CohS.I, "UPDATE_CACHE").reset,
    Event(CE.STORE, CohS.S) -> Do(CohS.SM_W, "ADD_TO_REQ_QUEUE").broadcast(R.UPG),
    Event(CE.OTHER_GETM, CohS.SM_W) -> Do(CohS.IM_W, "UPDATE_CACHE"),
    Event(CE.OTHER_UPG, CohS.SM_W) -> Do(CohS.IM_W, "UPDATE_CACHE"),
    ...
    Event(CE.OWN_UPG,    CohS.IM_W) -> Do(CohS.IM_AD, "ADD_TO_REQ_QUEUE").broadcast(R.GETM),
    // From M
    // No need for retry, as this line can be in M
    Event(CE.LOAD, CohS.M) -> Do(CohS.M, "ADD_TO_CACHE_RESP_QUEUE"),
    Event(CE.STORE, CohS.M) -> Do(CohS.M, "ADD_TO_CACHE_RESP_QUEUE_STORE"),
    Event(CE.REPLACEMENT, CohS.M) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),
    Event(CE.REPLACEMENT, CohS.M) -> Do(CohS.MI_WB, "ADD_TO_WB_QUEUE").broadcast(R.PUTM),
  ...
  override def getCoherenceWBStates: List[Int] = List(CohS.MS_A, CohS.MI_A, CohS.MI_WB,
    CohS.MS_WB, CohS.MS_WB_E, CohS.MI_WB_E, CohS.MS_A_E/*, CohS.EI_A, CohS.ES_A*/)
}
